...
export default function ShipmentLabel({ shipment, trailer, labelDate }: ShipmentLabelProps) {
  ...
  return (
    <div className="p-3 border border-foreground rounded-md shadow-sm w-full sm:w-[calc(50%-0.5rem)] md:w-[calc(33.333%-0.66rem)] lg:w-[calc(25%-0.75rem)] bg-background text-foreground print:shadow-none print:border-black print:w-[108mm] print:h-[150mm] print:p-3 print:text-[10pt] print:break-words label-item flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-center mb-1 print:mb-2">
            <h3 className="text-xs print:text-[18pt] font-bold text-primary print:text-black">Shipment Label</h3>
        ...
        <div className="space-y-1 print:space-y-0.5 print:my-px text-[0.65rem] print:text-[10pt] leading-normal print:leading-normal">
          <p className="flex items-center print:text-[14pt] print:mb-1"><CalendarDays className="h-3 w-3 mr-1 print:hidden"/><strong>Date:</strong> <span className="ml-1">{labelDate}</span></p>
          <p className="flex items-center print:text-[16pt] print:font-semibold print:mb-1"><Briefcase className="h-3 w-3 mr-1 print:hidden"/><strong>Agent:</strong> <span className="ml-1 truncate" title={trailer.company || 'N/A'}>{trailer.company || 'N/A'}</span></p>
          <p className="flex items-center print:text-[14pt] print:mb-1"><Users className="h-3 w-3 mr-1 print:hidden"/><strong>Importer:</strong> <span className="ml-1 truncate" title={shipment.importer}>{shipment.importer}</span></p>
          <p className="flex items-center print:text-[22pt] print:font-bold print:mb-2"><Tag className="h-3 w-3 mr-1 print:hidden"/><strong>Pieces:</strong> <span className="ml-1">{shipment.quantity}</span></p>
          
          <p className="flex items-center print:text-[18pt] print:font-bold print:mb-1">
            <strong>Ref:</strong> 
          ...
          {shipment.customerJobNumber && (
            <p className="flex items-center print:text-[16pt] print:mb-1">
      ...
      <div className="mt-2 pt-2 border-t border-dashed border-muted-foreground print:border-black print:mt-3 print:pt-2 print:mb-1">
        <p className="text-[0.6rem] print:text-[14pt] font-semibold text-center print:mb-1">QR CODE</p>
        ...
        <p className="text-center font-mono text-[0.55rem] print:text-[16pt] print:font-bold break-all mt-1 print:mt-1 leading-tight tracking-tighter" title={barcodeValue}>
          {barcodeValue}
        </p>
      </div>
    </div>
